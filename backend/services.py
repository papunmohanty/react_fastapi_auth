...
JWT_SECRET = "asupersecurejwtsecretkey"
...
async def _lead_selector(lead_id: int, user: _schemas.User, db: _orm.Session):
    """
    Reusable lead getter function to use in various parts where lead object
    based on lead id is needed.
    """
    lead = (
    ...
    if lead is None:
        raise _fastapi.HTTPException(
            status_code=404, detail="Lead does not exist"
        )
